...
export default function Communication() {
   ...
   * 在首次加载编辑器读取缓存的数据
   */
  const [content, setcontent] = useState(BraftEditor.createEditorState(React.$storage.get({key:"CurrentPage"}) || ""));
  ...
  const handleToSave = () => {
    ...
    setcallBack({func:()=>{React.$storage.set({key:"CurrentPage", value: content});}});
  ...
  return (
    <Fragment>
      <PageHeader
        ...
        onBack={() => (history.push("/home/center"))}
        title={title}
